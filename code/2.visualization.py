...
# Plot Temp against Evaporation
plt.scatter(melb_df["Mean Temperature (°C)"], melb_df["Evaporation (mm)"])
plt.xlabel("Mean temperature (°C)")
plt.ylabel("Evaporation (mm)")
plt.title("Victoria")
plt.subplots_adjust(bottom=0.3)
plt.savefig("melb_temp_evap.png")
plt.close()
...
# VISUALIZE MAX TEMPERATURE AGAINST PRICE DEMAND
melb_df = pd.read_csv("melb_weather_normalized.csv")
pdd_melb_df = pd.read_csv("pdd_melb_normalized.csv")
...
#maxtemp_demand.to_csv("norm_maxtemp_demand_melb.csv")
 
plt.scatter(maxtemp_demand["Maximum temperature (°C)"], maxtemp_demand["Normalized"])
plt.xlabel("Max temperature (°C)")
plt.ylabel("Total Energy Demand")
plt.title("Victoria")
plt.subplots_adjust(bottom=0.3)
plt.savefig("norm_maxtemp_demand_melb.png")
plt.close()
...
maxtemp_demand = pd.merge(melb_df[["Date", "Maximum temperature (°C)"]], pdd_melb_df, how='inner')
maxtemp_demand.set_index("Date", inplace=True)
#maxtemp_demand.to_csv("unnorm_maxtemp_demand_melb.csv")
